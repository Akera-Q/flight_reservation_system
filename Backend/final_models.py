...
#     def __init__(self, session, passenger_national_id: str, departure_country_code: str, destination_country_code: str, seat_number: str):
#         """
#         Initialize a new reservation.
#         """
#         # Find the passenger by their national ID
#         passenger = session.query(Passenger).filter_by(national_id=passenger_national_id).first()
#         if not passenger:
#             raise ValueError(f"No passenger found with National ID: {passenger_national_id}")
#         self.passenger = passenger

#         # Find the flight based on departure and destination country codes
#         flight = self.find_flight(session, departure_country_code, destination_country_code)
#         if not flight:
#             raise ValueError(f"No flight found from {departure_country_code} to {destination_country_code}")
#         self.flight = flight

#         # Set other attributes
#         self.seat_number = seat_number
#         self.status = "Pending"
#         self.departure_country = departure_country_code
#         self.destination_country = destination_country_code

#         # Save the reservation to the database
#         session.add(self)
#         session.commit()
#         print(f"Reservation created successfully with ID: {self.id}")
...
#         self.final_price = None  # Defer calculation until ticket_number is available
...
# # Seat class doesn't seem to logically have these methods in it
#     # @staticmethod
#     # def calculate_empty_seats(session, flight_id: int):
#     #     empty_seats = session.query(Seat).filter_by(flight_id=flight_id, is_available=True).count()
